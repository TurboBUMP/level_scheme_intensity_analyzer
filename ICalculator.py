...
def efficiency_error(energy):
    mask = np.isin(eff_vector[:,0],int(energy))
    # Efficiency error cannot be under 10%
    return max(eff_vector[mask][0,2],eff_vector[mask][0,1]*0.1)
    #return eff_vector[mask][0,2]

def efficiency_error_percentage(energy):
    mask = np.isin(eff_vector[:,0],int(energy))
    return (eff_vector[mask][0,2])/(eff_vector[mask][0,1])*100
...
def find_outgoing(energy_level):
    return lvl_scheme[lvl_scheme['LevelLITERATURE']==energy_level]
...
if __name__ == '__main__':
    ...
    if parser_arguments.efficiency is not None:
        energies = np.linspace(10,10000,100000)
        eff = np.asarray([efficiency(e) for e in energies])
        eff_error = np.asarray([efficiency_error(e) for e in energies])
        eff_error_perc = [efficiency_error_percentage(e) for e in energies]
